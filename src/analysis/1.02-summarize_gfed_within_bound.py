...
def summarize_gfed_within_boundary(boundary_path, output_file):
    ...
    """
    gfed_dir = os.path.join("data", "fire", "GFED5.1", "GFED5.1_ecosystem")
    ...
    gfed_files = glob.glob(os.path.join(gfed_dir, "GFED5.1_ecosystem_*.nc"))
    gfed_files.sort()
    ...
    for gfed_file in gfed_files:
        ...
        for month in range(1, 13):
            ...
            for var in emission_vars:
                ...
                arr = arr
                arr = arr.where(mask)
                value_sums = float(arr.sum().values)
                results.append(
                    {
                        "year": year,
                        "variable": var,
...
if __name__ == "__main__":
    ...
    output_file = os.path.join("outputs", "gfed_emissions_and_ba_in_alaska.csv")
